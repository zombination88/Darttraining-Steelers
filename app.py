    ...
    for b_name in boards_in_r:
        ...
        if is_coop_round or modus == "Koop 2vs2 (Up & Down)":
            ...
            with col_t1:
                ...
                sel_p1a = st.selectbox("Heim Spieler 1", available_players, index=idx1, key=f"qe_{b_name}_{chosen_r}_p1a_{session_idx}")
                idx2 = available_players.index(p1_b) if p1_b in available_players else (1 if len(available_players)>1 else 0)
                sel_p1b = st.selectbox("Heim Spieler 2", available_players, index=idx2, key=f"qe_{b_name}_{chosen_r}_p1b_{session_idx}")
            ...
            with col_t2:
                ...
                idx3 = available_players.index(p2_a) if p2_a in available_players else (2 if len(available_players)>2 else 0)
                sel_p2a = st.selectbox("Gast Spieler 1", available_players, index=idx3, key=f"qe_{b_name}_{chosen_r}_p2a_{session_idx}")
                idx4 = available_players.index(p2_b) if p2_b in available_players else (3 if len(available_players)>3 else 0)
                sel_p2b = st.selectbox("Gast Spieler 2", available_players, index=idx4, key=f"qe_{b_name}_{chosen_r}_p2b_{session_idx}")
            ...
            with col_p1:
                ...
                final_s1 = st.selectbox(f"Heim ({b_name})", available_players + ["-"], index=idx_s1 if curr_s1 in available_players else len(available_players), key=f"qe_{b_name}_{chosen_r}_s1_{session_idx}")
                in_l1 = st.number_input(f"Legs {final_s1}", min_value=0, max_value=5, value=s_l1, key=f"qe_l1_{b_name}_{chosen_r}_{session_idx}")
                in_180_1 = st.number_input(f"180er ({final_s1})", min_value=0, max_value=20, value=c1_180, key=f"qe_180_1_{b_name}_{chosen_r}_{session_idx}")
                
            with col_p2:
                idx_s2 = available_players.index(curr_s2) if curr_s2 in available_players else (1 if len(available_players)>1 else 0)
                final_s2 = st.selectbox(f"Gast ({b_name})", available_players + ["-"], index=idx_s2 if curr_s2 in available_players else len(available_players), key=f"qe_{b_name}_{chosen_r}_s2_{session_idx}")
                in_l2 = st.number_input(f"Legs {final_s2}", min_value=0, max_value=5, value=s_l2, key=f"qe_l2_{b_name}_{chosen_r}_{session_idx}")
                in_180_2 = st.number_input(f"180er ({final_s2})", min_value=0, max_value=20, value=c2_180, key=f"qe_180_2_{b_name}_{chosen_r}_{session_idx}")
...
with tab_übersicht:
    ...
    active_sessions_for_overview = [s for s in st.session_state.sessions_list if not is_session_completed(s)]
    curr_sess = active_sessions_for_overview[0] if active_sessions_for_overview else (st.session_state.sessions_list[0] if st.session_state.sessions_list else None)
