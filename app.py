...
with col2:
  st.markdown(
      "<h1 style='margin: 0; padding-top: 12px; font-size: 2.2rem;'>Wehringer"
      " Steeler - Teamtraining</h1>",
...
tab_übersicht, tab_kader, tab_session, tab_archiv = st.tabs(
    ["Übersicht", "Kader", "Session", "Match-Archiv"]
)
...
def get_boards_list(session, round_num=None):
  ...
  singles_rounds = session.get(
      "singles_rounds", total_rounds - 2 if total_rounds > 2 else 4
  )
  in_coop_phase = (
      is_standard_training and round_num is not None and round_num > singles_rounds
  )
...
def get_resting_player(session, round_num):
  ...
  if round_num == 1:
    return (
        spieler[2 * K]
        if len(spieler) > 2 * K
        else (spieler[-1] if len(spieler) % 2 != 0 else None)
    )
...
def get_board_players(session, round_num, board_name):
  ...
  if round_num == 1 and not in_coop_phase:
    ...
    if prev_sess and "results" in prev_sess:
      ...
      prev_singles = prev_sess.get(
          "singles_rounds",
          prev_total - 2 if prev_is_std and prev_total > 2 else prev_total,
      )
  ...
  if is_2v2 or in_coop_phase:
    ...
    if round_num == 1:
      ...
      while len(pairs) <= b_idx:
        pairs.append((
            spieler[0] if spieler else "-",
            spieler[1] if len(spieler) > 1 else "-",
        ))
      return list(pairs[b_idx])
...
def is_board_ready(session, board_name, next_r):
  ...
  singles_rounds = session.get(
      "singles_rounds",
      total_rounds - 2
      if modus == "Standard-Training (Einzel + Coop)" and total_rounds > 2
      else total_rounds,
  )
...
def open_substitution_dialog(
    board_name, session_idx, round_num, slot_num, current_player
):
  ...
  idx = (
      alle_spieler.index(current_player)
      if current_player in alle_spieler
      else 0
  )
  new_sel = st.selectbox(
      ...
      alle_spieler,
      index=idx,
      key=f"sub_sel_{board_name}_{round_num}_{slot_num}",
  )
  new_txt = st.text_input(
      "Oder neuen Gast eintragen:",
      placeholder="Name...",
      key=f"sub_txt_{board_name}_{round_num}_{slot_num}",
  )
  ...
  with col1:
    if st.button(
        ...
        key=f"sub_cancel_{board_name}_{round_num}_{slot_num}",
    ):
      st.rerun()
  with col2:
    if st.button(
        ...
        type="primary",
        use_container_width=True,
        key=f"sub_save_{board_name}_{round_num}_{slot_num}",
    ):
...
def open_session_archive_dialog(session_idx):
  ...
  st.caption(
      f"Modus: {sess['modus']} | Boards: {sess['boards']} | Leg-Modus:"
      f" {sess['modus_leg']}"
  )
  ...
  is_standard_training = modus == "Standard-Training (Einzel + Coop)"
  singles_rounds = sess.get(
      "singles_rounds",
      total_rounds - 2 if is_standard_training and total_rounds > 2 else total_rounds,
  )
    ...
    for r in range(1, total_rounds + 1):
      if is_standard_training:
        if r <= singles_rounds:
          ...
          r_title = (
              f"Doppelrunde {r - singles_rounds}/{total_rounds - singles_rounds}"
              " (Coop)"
          )
      else:
      ...
      for b_name in boards_in_r:
        ...
        if match_info:
          r_matches.append({
              ...
              "180er (H/G)": (
                  f"{match_info.get('180_s1', 0)} /"
                  f" {match_info.get('180_s2', 0)}"
              ),
              "Average (H/G)": (
                  f"{match_info.get('avg_s1', 0.0)} /"
                  f" {match_info.get('avg_s2', 0.0)}"
              ),
          })
        else:
...
def open_quick_entry_dialog(session_idx):
  pwd = st.text_input(
      "Passwort eingeben", type="password", key=f"qe_pwd_input_{session_idx}"
  )
  if pwd != "1521":
    if pwd != "":
    ...
    if is_standard_training:
      if r <= singles_rounds:
        ...
        round_options.append(
            (
                r,
                f"Doppelrunde {r - singles_rounds} /"
                f" {total_rounds - singles_rounds} (Coop)",
            )
        )
    else:
      round_options.append((r, f"Runde {r} / {total_rounds}"))

  selected_r_tuple = st.selectbox(
      "Wähle Runde aus:",
      round_options,
      format_func=lambda x: x[1],
      key=f"qe_round_sel_{session_idx}",
  )
  ...
  for b_name in boards_in_round:
    ...
    if is_coop_round_qe:
      h_parts = (
          [p.strip() for p in p1_default.split("&")]
          if "&" in p1_default
          else [p1_default, "-"]
      )
      g_parts = (
          [p.strip() for p in p2_default.split("&")]
          if "&" in p2_default
          else [p2_default, "-"]
      )
      ...
      with col_h1:
        idx_h1 = (
            alle_mögliche_spieler.index(h1_def)
            if h1_def in alle_mögliche_spieler
            else 0
        )
        sel_h1 = st.selectbox(
            f"Heim S1 ({b_name})",
            alle_mögliche_spieler,
            index=idx_h1,
            key=f"qe_h1_{session_idx}_{current_round}_{b_name}",
        )
      with col_h2:
        idx_h2 = (
            alle_mögliche_spieler.index(h2_def)
            if h2_def in alle_mögliche_spieler
            else 0
        )
        sel_h2 = st.selectbox(
            f"Heim S2 ({b_name})",
            alle_mögliche_spieler,
            index=idx_h2,
            key=f"qe_h2_{session_idx}_{current_round}_{b_name}",
        )
      ...
      with col_g1:
        idx_g1 = (
            alle_mögliche_spieler.index(g1_def)
            if g1_def in alle_mögliche_spieler
            else 0
        )
        sel_g1 = st.selectbox(
            f"Gast S1 ({b_name})",
            alle_mögliche_spieler,
            index=idx_g1,
            key=f"qe_g1_{session_idx}_{current_round}_{b_name}",
        )
      with col_g2:
        idx_g2 = (
            alle_mögliche_spieler.index(g2_def)
            if g2_def in alle_mögliche_spieler
            else 0
        )
        sel_g2 = st.selectbox(
            f"Gast S2 ({b_name})",
            alle_mögliche_spieler,
            index=idx_g2,
            key=f"qe_g2_{session_idx}_{current_round}_{b_name}",
        )

      sel_p1 = f"{sel_h1} & {sel_h2}"
      sel_p2 = f"{sel_g1} & {sel_g2}"
    else:
      if p1_default not in alle_mögliche_spieler:
        alle_mögliche_spieler.append(p1_default)
      if p2_default not in alle_mögliche_spieler:
        alle_mögliche_spieler.append(p2_default)
      alle_mögliche_spieler.sort()

      c_p1, c_vs, c_p2 = st.columns([3, 1, 3])
      with c_p1:
        idx1 = (
            alle_mögliche_spieler.index(p1_default)
            if p1_default in alle_mögliche_spieler
            else 0
        )
        sel_p1 = st.selectbox(
            f"Heim ({b_name})",
            alle_mögliche_spieler,
            index=idx1,
            key=f"qe_p1_{session_idx}_{current_round}_{b_name}",
        )
      with c_vs:
        st.markdown(
            "<div style='text-align: center; color: #ff4b4b; padding-top: 30px;"
            " font-weight: bold;'>VS</div>",
            unsafe_allow_html=True,
        )
      with c_p2:
        idx2 = (
            alle_mögliche_spieler.index(p2_default)
            if p2_default in alle_mögliche_spieler
            else 0
        )
        sel_p2 = st.selectbox(
            f"Gast ({b_name})",
            alle_mögliche_spieler,
            index=idx2,
            key=f"qe_p2_{session_idx}_{current_round}_{b_name}",
        )
    ...
    with sc1:
      s1_val = st.number_input(
          ...
          min_value=0,
          max_value=5,
          value=default_score1,
          key=f"qe_sc1_{session_idx}_{current_round}_{b_name}",
      )
    with sc2:
      s2_val = st.number_input(
          ...
          min_value=0,
          max_value=5,
          value=default_score2,
          key=f"qe_sc2_{session_idx}_{current_round}_{b_name}",
      )
    with t180_1:
      t180_1_val = st.number_input(
          "180er Heim",
          min_value=0,
          max_value=10,
          value=def_180_1,
          key=f"qe_180_1_{session_idx}_{current_round}_{b_name}",
      )
    with t180_2:
      t180_2_val = st.number_input(
          "180er Gast",
          min_value=0,
          max_value=10,
          value=def_180_2,
          key=f"qe_180_2_{session_idx}_{current_round}_{b_name}",
      )
    ...
    winner = (
        sel_p1
        if s1_val > s2_val
        else (sel_p2 if s2_val > s1_val else None)
    )
    loser = (
        sel_p2
        if winner == sel_p1
        else (sel_p1 if winner == sel_p2 else None)
    )
...
def open_new_session_dialog():
  ...
  spielmodus = st.selectbox(
      "Spielmodus",
      [
          "Standard-Training (Einzel + Coop)",
          "Up & Down",
          "Koop 2vs2 (Up & Down)",
          "Liga (4er-Team)",
      ],
  ...
  if spielmodus == "Standard-Training (Einzel + Coop)":
    ...
    singles_rounds = st.selectbox(
        "Anzahl Einzel-Runden", list(range(1, 11)), index=3
    )
    coop_rounds = st.selectbox(
        "Anzahl Doppel (Koop)-Runden", list(range(1, 5)), index=1
    )
    total_rounds = singles_rounds + coop_rounds
    st.info(
        f"ℹ️ Standard-Training: {singles_rounds} Runden Einzel (auf gewählten"
        f" Boards) + {coop_rounds} Runden Doppel/Koop (auf exakt 2 Boards)."
    )
  else:
    singles_rounds = 0
    coop_rounds = 0
  ...
  anzahl_boards = st.selectbox(
      "Anzahl der Boards (für Einzel)",
      [
          "4 Boards",
          "6 Boards",
          "5 Boards",
...
def open_retroactive_session_dialog():
  ...
  session_datum = st.date_input(
      "Datum der Session", yesterday, key="retro_date_input"
  )
  leg_modus = st.selectbox(
      "Leg-Modus", ["Best of 5", "Best of 3"], key="retro_leg_input"
  )
  spielmodus = st.selectbox(
      "Spielmodus",
      [
  ...
  if spielmodus == "Standard-Training (Einzel + Coop)":
    st.write("### Runden-Aufteilung")
    singles_rounds = st.selectbox(
        "Anzahl Einzel-Runden",
        list(range(1, 11)),
        index=3,
        key="retro_singles_input",
    )
    coop_rounds = st.selectbox(
        "Anzahl Doppel (Koop)-Runden",
        list(range(1, 5)),
        index=1,
        key="retro_coop_input",
    )
    total_rounds = singles_rounds + coop_rounds
  else:
    singles_rounds = 0
    coop_rounds = 0
    total_rounds = st.selectbox(
        "Anzahl Runden", list(range(1, 11)), index=3, key="retro_rounds_input"
    )
...
def open_edit_session_dialog(session_idx):
  pwd = st.text_input(
      "Passwort eingeben", type="password", key=f"edit_pwd_input_{session_idx}"
  )
  if pwd != "1521":
    if pwd != "":
      st.error("Falsches Passwort!")
  ...
  try:
    curr_date = pd.to_datetime(
        sess.get("datum", ""), format="%d.%m.%Y"
    ).date()
  except:
    curr_date = date.today()

  session_datum = st.date_input(
      "Datum", curr_date, key=f"edit_date_{session_idx}"
  )
  leg_modus = st.selectbox(
      "Leg-Modus",
      ["Best of 5", "Best of 3"],
      index=["Best of 5", "Best of 3"].index(sess.get("modus_leg", "Best of 5")),
      key=f"edit_leg_{session_idx}",
  )
  ...
  if curr_modus not in modi_list:
    modi_list.append(curr_modus)
  spielmodus = st.selectbox(
      "Spielmodus",
      modi_list,
      index=modi_list.index(curr_modus),
      key=f"edit_modus_{session_idx}",
  )

  if spielmodus == "Standard-Training (Einzel + Coop)":
    curr_total = sess.get("total_rounds", 6)
    curr_singles = sess.get(
        "singles_rounds", curr_total - 2 if curr_total > 2 else 4
    )
    ...
    singles_rounds = st.selectbox(
        "Anzahl Einzel-Runden",
        list(range(1, 11)),
        index=curr_singles - 1 if 1 <= curr_singles <= 10 else 3,
        key=f"edit_singles_{session_idx}",
    )
    coop_rounds = st.selectbox(
        "Anzahl Doppel (Koop)-Runden",
        list(range(1, 5)),
        index=curr_coop - 1 if 1 <= curr_coop <= 4 else 1,
        key=f"edit_coop_{session_idx}",
    )
    total_rounds = singles_rounds + coop_rounds
  else:
    singles_rounds = 0
    ...
    total_rounds = st.selectbox(
        "Anzahl Runden",
        list(range(1, 11)),
        index=sess.get("total_rounds", 4) - 1,
        key=f"edit_rounds_{session_idx}",
    )
  ...
  if curr_b not in board_opts:
    board_opts.append(curr_b)
  anzahl_boards = st.selectbox(
      "Anzahl der Boards",
      board_opts,
      index=board_opts.index(curr_b),
      key=f"edit_bcount_{session_idx}",
  )
  ...
  with cols[0]:
    for spieler in kader[:half]:
      if st.checkbox(
          spieler,
          value=(spieler in curr_spieler),
          key=f"edit_kader_{spieler}_{session_idx}",
      ):
        anwesende.append(spieler)
  with cols[1]:
    for spieler in kader[half:]:
  ...
  g1 = st.text_input(
      "Gastspieler 1",
      value=curr_gaeste[0] if len(curr_gaeste) > 0 else "",
      key=f"edit_gast1_{session_idx}",
  )
  g2 = st.text_input(
      "Gastspieler 2",
      value=curr_gaeste[1] if len(curr_gaeste) > 1 else "",
      key=f"edit_gast2_{session_idx}",
  )
  g3 = st.text_input(
      "Gastspieler 3",
      value=curr_gaeste[2] if len(curr_gaeste) > 2 else "",
      key=f"edit_gast3_{session_idx}",
  )
  g4 = st.text_input(
      "Gastspieler 4",
      value=curr_gaeste[3] if len(curr_gaeste) > 3 else "",
      key=f"edit_gast4_{session_idx}",
  )
...
def open_board_dialog(board_name, session_idx):
  ...
  res = sess.get("results", {})
  completed_rounds = [
      r for (r, b), v in res.items() if b == board_name and v.get("winner")
  ]
  ...
  if is_standard_training:
    if current_round <= singles_rounds:
      ...
      round_title_str = (
          f"Doppelrunde {current_round - singles_rounds}/"
          f"{total_rounds - singles_rounds} (Coop)"
      )
  else:
  ...
  with col1:
    ...
    in_score1 = st.number_input(
        "Legs Heim",
        min_value=0,
        max_value=5,
        value=score1,
        key=f"d_score1_{board_name}_{session_idx}",
    )
    in_180_1 = st.number_input(
        f"🎯 180er von {current_p1}",
        min_value=0,
        max_value=20,
        value=t1_180,
        key=f"d_180_1_{board_name}_{session_idx}",
    )
    in_avg_1 = st.number_input(
        f"📊 Match-Average {current_p1}",
        min_value=0.0,
        max_value=180.0,
        value=avg1,
        step=0.1,
        key=f"d_avg_1_{board_name}_{session_idx}",
    )

  with col2:
    st.markdown(f"**Gast:** `{current_p2}`")
    in_score2 = st.number_input(
        "Legs Gast",
        min_value=0,
        max_value=5,
        value=score2,
        key=f"d_score2_{board_name}_{session_idx}",
    )
    in_180_2 = st.number_input(
        f"🎯 180er von {current_p2}",
        min_value=0,
        max_value=20,
        value=t2_180,
        key=f"d_180_2_{board_name}_{session_idx}",
    )
    in_avg_2 = st.number_input(
        f"📊 Match-Average {current_p2}",
        min_value=0.0,
        max_value=180.0,
        value=avg2,
        step=0.1,
        key=f"d_avg_2_{board_name}_{session_idx}",
    )
  ...
  st.info(
      f"📊 Ergebnis: **{ergebnis}** | 🏆 Sieger:"
      f" **{winner if winner else 'Unentschieden'}**"
  )
  ...
  with col_btn1:
    if st.button(
        ...
        type="primary",
        use_container_width=True,
        key=f"d_save_{board_name}_{session_idx}",
    ):
      if in_score1 == in_score2:
...
def open_delete_dialog(session_idx):
  ...
  sess = st.session_state.sessions_list[session_idx]
  st.warning(
      f"Soll die Session **{sess['id']}** vom **{sess['datum']}** wirklich"
      " unwiderruflich gelöscht werden?"
  )

  pwd = st.text_input(
      "Passwort zur Bestätigung", type="password", key=f"del_pwd_input_{session_idx}"
  )
...
with tab_übersicht:
  ...
  with col_btn2:
    active_sessions_for_btn = [
        s for s in st.session_state.sessions_list if not is_session_completed(s)
    ]
    if active_sessions_for_btn:
      if st.button(
          "⚙️ Aktive Session bearbeiten",
          use_container_width=True,
          key="edit_active_btn",
      ):
        open_edit_session_dialog(
            st.session_state.sessions_list.index(active_sessions_for_btn[0])
        )
    else:
      st.button(
          "⚙️ Aktive Session bearbeiten", use_container_width=True, disabled=True
      )
  ...
  target_sess_for_metrics = (
      active_sessions_for_btn[0]
      if active_sessions_for_btn
      else (
          st.session_state.sessions_list[0]
  ...
  for sess in st.session_state.sessions_list:
    for m_inf in sess.get("results", {}).values():
      total_180s_count += int(m_inf.get("180_s1", 0)) + int(
          m_inf.get("180_s2", 0)
      )

  current_active_players_count = (
      len(target_sess_for_metrics.get("spieler", kader))
      if target_sess_for_metrics
      else len(kader)
  )
  ...
  with col1:
    st.metric(
        label="Trainingsabende", value=str(total_sessions_count), delta="Gesamt"
    )
  with col2:
    st.metric(
        label="Team 180er", value=str(total_180s_count), delta="geworfen 🎯"
    )
  with col3:
    st.metric(
        label="Anwesende Spieler",
        value=str(current_active_players_count),
        delta="heute" if active_sessions_for_btn else "im Kader",
    )
  with col4:
    st.metric(
        label="Aktueller Kaiser 👑",
        value=active_kaiser_name,
        delta="Board 1",
    )
  ...
  if not active_sessions_for_btn:
    st.info(
        "Derzeit läuft keine aktive Session. Starte eine neue Session, um die"
        " Übersicht zu sehen."
    )
  else:
    curr_sess = active_sessions_for_btn[0]
    st.caption(
        f"Session-ID: **{curr_sess['id']}** vom {curr_sess['datum']}"
        f" ({curr_sess['modus']})"
    )
    ...
    curr_waiting = get_waiting_player(curr_sess, current_active_round)
    if curr_waiting and curr_waiting != "-":
      st.info(
          f"☕ **Pause in dieser Runde:** `{curr_waiting}` (Wartet auf das Ende"
          " des letzten Boards)"
      )
    ...
    if is_standard_training:
      if current_active_round <= singles_rounds:
        round_title_str = (
            f"Runde {current_active_round} von {singles_rounds} (Einzel)"
        )
      else:
        round_title_str = (
            f"Doppelrunde {current_active_round - singles_rounds} von"
            f" {total_rounds - singles_rounds} (Coop)"
        )
    else:
    ...
    st.markdown(
        f"#### {round_title_str} ({len(all_possible_boards)} Boards aktiv)"
    )
        ...
        if i + j < len(all_possible_boards):
          ...
          with cols[j]:
            with st.container(border=True):
              ...
              st.markdown(
                  f"<h4 style='text-align: center; margin-bottom:"
                  f" 0;'>{b_name}</h4>",
                  unsafe_allow_html=True,
              )
              ...
              if next_r <= total_rounds:
                ...
                if b_name not in session_boards_at_r:
                  st.markdown(
                      ...
                      " 0.85em;'>Nicht in dieser Phase</p>",
                      unsafe_allow_html=True,
                  )
                else:
                  ...
                  st.markdown(
                      ...
                      f" font-size: 1.1em; margin-top: 5px; margin-bottom:"
                      f" 0;'>{ampel}</p>",
                      unsafe_allow_html=True,
                  )
                  ...
                  if is_standard_training:
                    if next_r <= singles_rounds:
                      ...
                      round_sub_str = (
                          f"Doppelrunde {next_r - singles_rounds}/"
                          f"{total_rounds - singles_rounds}"
                      )
                  else:
                  ...
                  if ready and p1 != "-" and p1 != "Offen":
                    ...
                    sc1.markdown(
                        ...
                        f" padding-top: 5px;'>{p1}</div>",
                        unsafe_allow_html=True,
                    )
                    with sc2:
                      if st.button(
                          "🔄 Ändern",
                          key=f"sub_btn1_{b_name}_{next_r}",
                          help="Spieler 1 auswechseln",
                      ):
                        open_substitution_dialog(
                            b_name,
                            st.session_state.sessions_list.index(curr_sess),
                            next_r,
                            1,
                            p1,
                        )
                    ...
                    st.markdown(
                        ...
                        f" font-size: 0.95em; margin: 12px 0;'>{p1}</div>",
                        unsafe_allow_html=True,
                    )

                  st.markdown(
                      "<div style='text-align: center; color: #ff4b4b;"
                      " font-size: 0.9em; margin: 2px 0;'>VS</div>",
                      unsafe_allow_html=True,
                  )
                  ...
                  if ready and p2 != "-" and p2 != "Offen":
                    ...
                    sc3.markdown(
                        "<div style='font-weight: bold; font-size: 0.95em;"
                        f" padding-top: 5px;'>{p2}</div>",
                        unsafe_allow_html=True,
                    )
                    with sc4:
                      if st.button(
                          "🔄 Ändern",
                          key=f"sub_btn2_{b_name}_{next_r}",
                          help="Spieler 2 auswechseln",
                      ):
                        open_substitution_dialog(
                            b_name,
                            st.session_state.sessions_list.index(curr_sess),
                    ...
                    st.markdown(
                        "<div style='text-align: center; font-weight: bold;"
                        f" font-size: 0.95em; margin: 12px 0;'>{p2}</div>",
                        unsafe_allow_html=True,
                    )
                  ...
                  if st.button(
                      "🎯 Ergebnis eintragen",
                      key=f"live_btn_{b_name}_{next_r}",
                      use_container_width=True,
                      disabled=not ready,
                  ):
                    open_board_dialog(
                        b_name, st.session_state.sessions_list.index(curr_sess)
                    )
              else:
                st.markdown(
                    "<p style='text-align: center; color: gray; font-size:"
                    f" 0.85em;'>Alle {total_rounds} Runden beendet</p>",
                    unsafe_allow_html=True,
                )
    ...
    if display_sess:
      ...
      for m in l_results.values():
        ...
        if s1_name and " & " not in s1_name:
          count_180s[s1_name] = count_180s.get(s1_name, 0) + c1
        if s2_name and " & " not in s2_name:
          count_180s[s2_name] = count_180s.get(s2_name, 0) + c2

        if s1_name and " & " not in s1_name and a1 > 0:
          match_avgs.append((s1_name, a1))
        if s2_name and " & " not in s2_name and a2 > 0:
          match_avgs.append((s2_name, a2))
  ...
  with col_r:
    ...
    st.markdown(
        f"**{best_p}** (Siegquote: {(best_q*100):.0f}% bei {best_m} Matches)"
    )
    st.progress(best_q)
...
with tab_kader:
  ...
  st.write(
      "Live berechnete Bilanz des festen Stammkaders (exklusive Gastspieler)"
      " inklusive Legs, 180er, Match-Averages und Gesamtschnitt des Teams."
  )
  ...
  for sess in st.session_state.sessions_list:
    ...
    if sess_avgs:
      ...
      team_session_avgs.append(
          {"Datum": sess.get("datum", "Unbekannt"), "Team-Average": round(t_avg, 1)}
      )

  total_games = total_wins + total_losses
  avg_win_rate = (
      f"{(total_wins / total_games * 100):.0f}%" if total_games > 0 else "0%"
  )

  all_team_avgs = [
      stats[p]["Avg_Sum"] / stats[p]["Avg_Count"]
      for p in kader
      if stats[p]["Avg_Count"] > 0
  ]
  overall_team_avg = (
      f"{(sum(all_team_avgs) / len(all_team_avgs)):.1f}"
      if all_team_avgs
      else "–"
  )
  ...
  with col2:
    st.metric(
        label="Absolvierte Matches",
        value=str(player_matches_played),
        delta="aus Sessions",
    )
  with col3:
  ...
  with col4:
    st.metric(
        label="Team-Gesamtschnitt", value=overall_team_avg, delta="Ø Average"
    )
  ...
  if team_session_avgs:
    ...
    st.info(
        "Noch nicht genügend Average-Daten vorhanden, um die Team-Entwicklung"
        " anzuzeigen."
    )
  ...
  st.caption(
      "Auswertung aller Doppel- und Koop-Matches zur Findung der perfekten"
      " Ligapaarungen."
  )
  ...
  for sess in st.session_state.sessions_list:
    for match in sess.get("results", {}).values():
      ...
      def process_pair(
          pair_str, is_won, won_legs, lost_legs, h_count, avg_val
      ):
        if " & " in pair_str:
...
with tab_session:
  ...
  with col1:
    st.metric(
        label="Gespielte Abende",
        value=str(len(st.session_state.sessions_list)),
        delta="gefilterte Sessions",
    )
  with col2:
  ...
  if not st.session_state.sessions_list:
    ...
    for idx, sess in enumerate(st.session_state.sessions_list):
      with st.container():
        gaeste_text = (
            f" | Gäste: {', '.join(sess['gaeste'])}"
            if sess.get("gaeste")
            else ""
        )
        status_text = (
            " ✅ **[Abgeschlossen]**" if is_session_completed(sess) else ""
        )
        total_rounds = sess.get("total_rounds", 4)
        st.markdown(
            f"**{sess['datum']}** — *{sess['modus']} · {sess['boards']} ·"
            f" {total_rounds} Runden · {sess['modus_leg']} ·"
            f" {sess['id']}{gaeste_text}*{status_text}"
        )
          ...
          with b_cols[b_i]:
            ...
            if is_std_s:
              if next_r <= singles_r_s:
                ...
                lbl_r_str = (
                    f"Doppelrunde {next_r - singles_r_s}/"
                    f"{total_rounds - singles_r_s}"
                )
            else:
...
with tab_archiv:
  ...
  st.write(
      "Klicke auf eine Session, um den detaillierten Spielablauf aller Runden"
      " einzusehen, oder verwalte/lösche sie bei Bedarf."
  )

  col_a1, col_a2 = st.columns(2)
  with col_a1:
    if st.button(
        "➕ Vergangene Session nachtragen",
        type="primary",
        use_container_width=True,
        key="retro_session_main_btn",
    ):
  ...
  if not st.session_state.sessions_list:
    ...
    for idx, sess in enumerate(st.session_state.sessions_list):
      with st.container(border=True):
        ...
        total_rounds = sess.get("total_rounds", 4)
        st.markdown(
            f"**{sess['id']}** — {sess['datum']} (*{modus_txt} · {boards_txt} ·"
            f" {total_rounds} Runden*{gaeste_text})"
        )
