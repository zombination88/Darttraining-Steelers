...
def load_chunked(ws):
try:
# Reparierte Such-Logik (Sucht gezielt in der json_data Spalte)
...
```
    # Entfernt gigantische Fotodateien aus dem Backup-Log, um das 50.000 Zeichen-Limit pro Zelle nicht zu sprengen
    slim_sessions = []
...
if g["heim"]: cell_content.append(Paragraph(f"Heim vs{g['gegner']}", event_style_heim))
else: cell_content.append(Paragraph(f"Ausw. @{g['gegner']}", event_style_gast))
...
curr_year, curr_month = min_d.year, min_d.month
while True:
html_blocks.append(f"{month_names[curr_month]} {curr_year}")
for dw in days_of_week: html_blocks.append(f"{dw}")
for week in calendar.monthcalendar(curr_year, curr_month):
for day in week:
if day == 0: html_blocks.append("")
else:
curr_date = date(curr_year, curr_month, day)
content = f"{day}"
if curr_date in games_by_date:
...
in_score1 = st.number_input("Legs Heim", 0, 5, score1, key=f"score1_{session_id}*{board_name}")
in_180_1 = st.number_input("🎯 180er Heim", 0, 20, t1_180, key=f"180_1*{session_id}*{board_name}")
in_avg_1 = st.number_input("📊 Avg Heim", 0.0, 180.0, avg1, step=0.1, key=f"avg_1*{session_id}*{board_name}")
with c2:
st.markdown(f"**Gast:** `{current_p2}`")
in_score2 = st.number_input("Legs Gast", 0, 5, score2, key=f"score2*{session_id}*{board_name}")
in_180_2 = st.number_input("🎯 180er Gast", 0, 20, t2_180, key=f"180_2*{session_id}*{board_name}")
in_avg_2 = st.number_input("📊 Avg Gast", 0.0, 180.0, avg2, step=0.1, key=f"avg_2*{session_id}_{board_name}")
...
new_sel = st.selectbox("Aus Kader wählen:", alle_spieler, index=idx, key=f"sub_sel_{session_id}*{board_name}*{round_num}*{slot_num}")
new_txt = st.text_input("Oder neuen Gast eintragen:", placeholder="Name...", key=f"sub_txt*{session_id}*{board_name}*{round_num}_{slot_num}")
...
st.markdown(f"{b_name}{m_str}🥇 1. Platz: {platz1}🥈 2. Platz: {platz2}", unsafe_allow_html=True)
st.divider()
...
st.markdown(f"{medal} Platz {rank}: {team_name}Siege: {stats['wins']} | Legs: {stats['legs_won']}:{stats['legs_lost']}", unsafe_allow_html=True)
rank += 1
if st.button("Schließen", use_container_width=True): st.rerun()
...
with c_title: st.markdown("Wehringer Steelers — Teamtraining", unsafe_allow_html=True)
