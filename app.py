...
# --- LOGIN & ROLLEN-SYSTEM (MAGIC LINK) ---
auth_val = ""
try:
    auth_val = st.query_params.get("auth", "")
except:
    try:
        auth_val = st.experimental_get_query_params().get("auth", [""])[0]
    except:
        pass

if "role" not in st.session_state:
    if auth_val == "20Steelers25":
    ...
    if st.session_state.role == "Gast":
        ...
        if st.button("Einloggen", use_container_width=True):
            if pwd == "20Steelers25" or pwd == "1521":
                st.session_state.role = "Spieler"
                try:
                    st.query_params["auth"] = "20Steelers25"
                except:
                    try: st.experimental_set_query_params(auth="20Steelers25")
                    except: pass
                st.rerun()
            else:
                st.error("Falsches Passwort!")
        st.markdown("---")
        st.caption("💡 **Tipp fürs Handy:** Logge dich einmal ein. Die Internetadresse ändert sich dann automatisch. Speichere dir die Seite DANN als Lesezeichen oder auf den Homescreen. Die App merkt sich dich für immer!")
    else:
...
def check_session_completion_time(sess):
    if sess.get("is_liga") or sess.get("is_wettkampf"):
        return
...
def open_wettkampf_blitz_dialog(session_id):
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            st.error(f"Bild konnte nicht komprimiert werden: {e}")
    ...
    for m_key, label, h_key, g_key in match_plan:
        ...
        with st.expander(f"{label}: {def_h} vs {def_g}", expanded=False):
            ...
            res[m_key] = {
                ...
                "ind_180_h": ind_180_h, "ind_180_g": ind_180_g,
                "hf_h": h_hf, "hf_g": g_hf,
                "sl_h": h_sl, "sl_g": g_sl
            }
...
def open_wettkampf_view_dialog(session_id):
    ...
    for m_key, label, h_key, g_key in match_plan:
        ...
        if m_data.get("played"):
            ...
            if h_str or g_str:
                st.caption(f"Highlights: Heim [{h_str}] | Gast [{g_str}]")
            st.write("")
        ...
        with st.expander("📊 Spieler-Statistik (Gesamte Saison)", expanded=True):
            ...
            for p in kader:
                if l_stats[p]["Matches"] > 0:
                    ...
                    table_rows.append({
                        "Spieler": p, "Matches": l_stats[p]["Matches"], "Siege": l_stats[p]["Siege"],
                        "Quote": f"{(l_stats[p]['Siege'] / l_stats[p]['Matches'] * 100):.0f}%",
                        "Legs": f"{l_stats[p]['Legs_Won']}:{l_stats[p]['Legs_Lost']}",
                        "180er": l_stats[p]["180er"], "High Finish": hf_str, "Bestes SL": sl_str
                    })
            if table_rows:
                st.dataframe(pd.DataFrame(table_rows).sort_values("Siege", ascending=False), hide_index=True)
            else:
                st.info("Noch keine abgeschlossenen Spiele zur Auswertung vorhanden.")
...
is_admin = st.session_state.role == "Spieler" # Schaltet alle Eingabefelder frei
# ------------------------------------------
