    ...
    if not st.session_state.sessions_list:
        ...
        for idx in sorted_indices:
            ...
            with st.container(border=True):
                ...
                if st.checkbox("⚡ Blitzeintrag & Korrektur (Admin)", key=f"blitz_check_{idx}"):
                    pwd_blitz = st.text_input("Passwort:", type="password", key=f"blitz_pwd_{idx}")
                    if pwd_blitz == "1521":
                        st.markdown("#### Schnelleingabe / Korrektur")
                        ...
                        for r in range(1, total_rounds + 1):
                            ...
                            for b_name in boards_in_r:
                                ...
                                if existing_match:
                                    ...
                                    p2 = existing_match.get("s2", "-")
                                    try:
                                        s1 = int(existing_match.get("ergebnis", "0:0").split(":")[0])
                                        s2 = int(existing_match.get("ergebnis", "0:0").split(":")[1])
                                    except:
                                        s1, s2 = 0, 0
                                else:
                                    auto_p = get_board_players(sess, r, b_name)
                                ...
                                with c1: st.write(p1)
                                with c2: in_s1 = st.number_input("", min_value=0, max_value=5, value=s1, key=f"b_s1_{idx}_{r}_{b_name}", label_visibility="collapsed")
                                with c3: in_s2 = st.number_input("", min_value=0, max_value=5, value=s2, key=f"b_s2_{idx}_{r}_{b_name}", label_visibility="collapsed")
                                with c4: st.write(p2)
                                
                                with c5:
                                    cc1, cc2 = st.columns(2)
                                    with cc1:
                                        if st.button("💾", key=f"b_save_{idx}_{r}_{b_name}", help="Speichern"):
                                            winner = p1 if in_s1 > in_s2 else (p2 if in_s2 > in_s1 else "-")
                                            loser = p2 if winner == p1 else (p1 if winner == p2 else "-")
                                            
                                            if "results" not in sess: sess["results"] = {}
                                            
                                            t1_180 = existing_match.get("180_s1", 0) if existing_match else 0
                                            t2_180 = existing_match.get("180_s2", 0) if existing_match else 0
                                            a1 = existing_match.get("avg_s1", 0.0) if existing_match else 0.0
                                            a2 = existing_match.get("avg_s2", 0.0) if existing_match else 0.0

                                            sess["results"][match_key] = {
                                                "s1": p1, "s2": p2,
                                                "ergebnis": f"{in_s1}:{in_s2}",
                                                "winner": winner, "loser": loser,
                                                "180_s1": t1_180, "180_s2": t2_180,
                                                "avg_s1": a1, "avg_s2": a2
                                            }
                                            smart_sync_and_save(st.session_state.sessions_list)
                                            st.success("Gespeichert!")
                                    with cc2:
                                        if st.button("🗑️", key=f"b_del_{idx}_{r}_{b_name}", help="Ergebnis löschen"):
                                            if "results" in sess and match_key in sess["results"]:
                                                del sess["results"][match_key]
                                                smart_sync_and_save(st.session_state.sessions_list)
                                                st.success("Gelöscht!")
                    elif pwd_blitz != "":
                        st.error("Falsches Passwort!")
