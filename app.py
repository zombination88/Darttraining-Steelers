...
#    - Koop 2vs2 (Up & Down): Reine Doppel-Session (0 Einzel). Gespielt wird exklusiv auf Kaiser B1 & Board 2 mit festen Teams.
#    - Up & Down (Einzel): Klassisch. Sieger steigt auf (Ri. B1), Verlierer ab. Start-Aufstellung: Kaiser der Vorsession startet ganz unten, neue Spieler ganz oben.
#    - Liga (4er-Team): Basis-Rundenablauf auf gewählter Board-Anzahl ohne automatische Team-Generierung.
        ...
        try:
            ...
            if len(all_vals) > 21: # 1 Kopfzeile + 20 Backups
                ...
                for _ in range(rows_to_delete):
                    try:
                        backup_ws.delete_rows(2) # Neuere gspread Versionen
                    except AttributeError:
                        backup_ws.delete_row(2)  # Ältere gspread Versionen
        except Exception:
            pass
...
def get_board_players(session, round_num, board_name):
    ...
    if round_num == 1 and not in_coop_phase and not is_2v2:
        all_sessions = sorted(st.session_state.sessions_list, key=lambda x: int(x['id'].split('-')[1]) if 'id' in x and '-' in x['id'] else 0, reverse=True)
        try:
...
with tab_kader:
    ...
    if valid_players:
        ...
        if len(top_mvps) == len(valid_players) and len(valid_players) > 1:
            mvp_player = "Alle gleichauf"
        elif len(top_mvps) <= 2:
            mvp_player = " & ".join(top_mvps)
        else:
    ...
    if not st.session_state.sessions_list:
        ...
        for idx, sess in enumerate(all_sessions_sorted):
            with st.container(border=True):
                ...
                st.markdown(f"**{sess['id']}** — {sess['datum']} ({time_str}) {status_text}")
