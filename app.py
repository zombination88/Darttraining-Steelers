...
# 6. KADER-STATS: Im Reiter Kader werden MVP, Dauerbrenner, Bester Avg und 180er Maschine als Hover/Tooltip angezeigt.
...
# 9. LIGA-BETRIEB: Komplett vom Training isolierter Modus (Schwaben 4. BezLiga) mit eigenem Spielbericht (10 Matches) und eigenen Stats.
...
# Daten strikt in Liga und Training trennen, damit sie sich nicht stören
...
tab_übersicht, tab_kader, tab_session, tab_liga, tab_archiv, tab_regeln = st.tabs(["Übersicht", "Kader", "Session", "Liga-Betrieb", "Match-Archiv", "Modus & Regeln"])
...
LIGA_MATCH_MAP = [
    ...
    ("m8", "Einzel 8", "h4", "g3"),
    ("m9", "Doppel 1", "hd1", "gd1"),
    ("m10", "Doppel 2", "hd2", "gd2"),
]

@st.dialog("➕ Neues Liga-Spiel (4. BezLiga)", width="large")
def open_new_liga_match_dialog():
    st.write("Erstelle hier ein neues Ligaspiel. Du kannst die Spieler-Namen auch später noch ergänzen.")
    c1, c2 = st.columns(2)
    ...
    with cb2:
        if st.button("Liga-Spiel erstellen", type="primary", use_container_width=True):
            ...
            new_session = {
                ...
                "heim_team": heim_team,
                "gast_team": gast_team,
                "aufstellung": {"h1": "-", "h2": "-", "h3": "-", "h4": "-", "g1": "-", "g2": "-", "g3": "-", "g4": "-", "hd1": "-", "hd2": "-", "gd1": "-", "gd2": "-"},
                "results": {}
            }
            st.session_state.sessions_list.append(new_session)
            smart_sync_and_save(st.session_state.sessions_list)
...
@st.dialog("📋 Aufstellung bearbeiten", width="large")
def open_edit_liga_aufstellung_dialog(session_idx):
    sess = liga_sessions[session_idx]
    real_idx = st.session_state.sessions_list.index(sess)
    auf = sess.get("aufstellung", {})
    
    c1, c2 = st.columns(2)
    with c1:
        st.write(f"### {sess.get('heim_team', 'Heim')}")
        h1 = st.selectbox("Heim 1", kader + ["-"], index=kader.index(auf.get("h1")) if auf.get("h1") in kader else len(kader))
        h2 = st.selectbox("Heim 2", kader + ["-"], index=kader.index(auf.get("h2")) if auf.get("h2") in kader else len(kader))
        h3 = st.selectbox("Heim 3", kader + ["-"], index=kader.index(auf.get("h3")) if auf.get("h3") in kader else len(kader))
        h4 = st.selectbox("Heim 4", kader + ["-"], index=kader.index(auf.get("h4")) if auf.get("h4") in kader else len(kader))
        hd1 = st.text_input("Heim Doppel 1", value=auf.get("hd1", "-"), placeholder="z.B. Andi & Marco")
        hd2 = st.text_input("Heim Doppel 2", value=auf.get("hd2", "-"), placeholder="z.B. Tobi & Max")
    with c2:
        st.write(f"### {sess.get('gast_team', 'Gast')}")
        g1 = st.text_input("Gast 1", value=auf.get("g1", "-"))
        g2 = st.text_input("Gast 2", value=auf.get("g2", "-"))
        g3 = st.text_input("Gast 3", value=auf.get("g3", "-"))
        g4 = st.text_input("Gast 4", value=auf.get("g4", "-"))
        gd1 = st.text_input("Gast Doppel 1", value=auf.get("gd1", "-"))
        gd2 = st.text_input("Gast Doppel 2", value=auf.get("gd2", "-"))

    cb1, cb2 = st.columns(2)
    if cb1.button("Abbrechen", use_container_width=True): st.rerun()
    if cb2.button("Aufstellung speichern", type="primary", use_container_width=True):
        sess["aufstellung"] = {"h1": h1, "h2": h2, "h3": h3, "h4": h4, "g1": g1, "g2": g2, "g3": g3, "g4": g4, "hd1": hd1, "hd2": hd2, "gd1": gd1, "gd2": gd2}
        st.session_state.sessions_list[real_idx] = sess
        smart_sync_and_save(st.session_state.sessions_list)
        st.rerun()

@st.dialog("📝 Offizieller Spielbericht (10 Matches)", width="large")
def open_liga_bericht_dialog(session_idx):
    ...
    st.write(f"Trage hier die Legs und Extras (180, High Finishes, Short Legs) für **{sess.get('heim_team')} vs {sess.get('gast_team')}** ein.")
    
    # 10 Matches rendern
    for m_key, label, h_key, g_key in LIGA_MATCH_MAP:
        p_heim, p_gast = auf.get(h_key, "-"), auf.get(g_key, "-")
        m_data = res.get(m_key, {})
        
        with st.expander(f"{label}: {p_heim} vs {p_gast}", expanded=False):
            c_lh, c_vs, c_lg = st.columns([2, 1, 2])
            lh = c_lh.number_input("Legs Heim", 0, 3, m_data.get("lh", 0), key=f"lh_{m_key}")
            c_vs.markdown("<div style='text-align: center; padding-top: 30px; font-weight: bold;'>:</div>", unsafe_allow_html=True)
            lg = c_lg.number_input("Legs Gast", 0, 3, m_data.get("lg", 0), key=f"lg_{m_key}")
            
            st.caption("Extras eintragen (nur Zahlen, z.B. '112' oder '15')")
            ce1, ce2 = st.columns(2)
            with ce1:
                e180_h = st.number_input("180er", 0, 10, m_data.get("180_h", 0), key=f"180h_{m_key}")
                hf_h = st.text_input("High Finish", m_data.get("hf_h", ""), key=f"hfh_{m_key}")
                sl_h = st.text_input("Short Leg", m_data.get("sl_h", ""), key=f"slh_{m_key}")
            with ce2:
                e180_g = st.number_input("180er", 0, 10, m_data.get("180_g", 0), key=f"180g_{m_key}")
                hf_g = st.text_input("High Finish", m_data.get("hf_g", ""), key=f"hfg_{m_key}")
                sl_g = st.text_input("Short Leg", m_data.get("sl_g", ""), key=f"slg_{m_key}")
                
            res[m_key] = {"lh": lh, "lg": lg, "180_h": e180_h, "hf_h": hf_h, "sl_h": sl_h, "180_g": e180_g, "hf_g": hf_g, "sl_g": sl_g}

    cb1, cb2 = st.columns(2)
    if cb1.button("Schließen", use_container_width=True): st.rerun()
    if cb2.button("💾 Spielbericht speichern", type="primary", use_container_width=True):
        sess["results"] = res
        st.session_state.sessions_list[real_idx] = sess
        smart_sync_and_save(st.session_state.sessions_list)
...
with tab_liga:
    ...
    st.write("Eigener Bereich für Ligaspielen nach dem offiziellen Spielbericht-System.")
    ...
    if not liga_sessions:
        ...
        for idx, l_sess in enumerate(sorted_liga):
            real_idx = liga_sessions.index(l_sess)
            heim = l_sess.get("heim_team", "Heim")
            gast = l_sess.get("gast_team", "Gast")
            res = l_sess.get("results", {})
            
            # Berechne Live Score
            sets_heim, sets_gast = 0, 0
            legs_heim, legs_gast = 0, 0
            ...
            is_done = len(res) == 10 and all((v.get("lh",0) > 0 or v.get("lg",0) > 0) for v in res.values())
            ...
            with st.container(border=True):
                ...
                c1, c2 = st.columns(2)
                with c1:
                    if st.button("📋 Aufstellung bearbeiten", key=f"l_auf_{l_sess['id']}", use_container_width=True):
                        open_edit_liga_aufstellung_dialog(real_idx)
                with c2:
                    if st.button("📝 Spielbericht", key=f"l_ber_{l_sess['id']}", use_container_width=True):
...
with tab_archiv:
    st.subheader("Match-Archiv (Training)")
    st.caption("JSON-Download inkludiert Training und Liga.")
    ...
    if not training_sessions:
        st.info("Keine Trainings-Sessions vorhanden.")
    else:
        sorted_sessions = sorted(training_sessions, key=lambda x: int(x["id"].split("-")[1]) if "id" in x and "-" in x["id"] else 0, reverse=True)
        for sess in sorted_sessions:
            orig_idx = training_sessions.index(sess)
            with st.container(border=True):
                ...
                c1, c2, c3 = st.columns(3)
                with c1:
                    if st.button("📊 Ansehen", key=f"arch_view_{sess['id']}", use_container_width=True): open_session_summary_dialog(orig_idx)
                with c2:
                    if st.button("⚙️ Bearbeiten", key=f"arch_edit_{sess['id']}", use_container_width=True): open_edit_session_dialog(orig_idx)
                with c3:
...
with tab_regeln:
    ...
    with st.container(border=True):
        ...
        st.markdown("""
        * Komplett isolierter Bereich im Tab **Liga-Betrieb**.
        * **Ablauf:** Zuerst 4 Einzel (1v1, 2v2, 3v3, 4v4), dann 4 überkreuzte Einzel (1v2, 2v1, 3v4, 4v3), dann 2 Doppel.
        * Das Eintragen der Legs, 180er, High Finishes und Short Legs geht direkt über den **📝 Spielbericht**-Button ohne Passwort.
        """)
