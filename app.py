...
# WICHTIG: Kader-Definition ganz oben, damit Dialoge jederzeit darauf zugreifen können!
kader = [
...
def save_backup_to_cloud(serializable_sessions):
    try:
        ...
        # Wichtig: Base64-Bilder rausschneiden, damit Zelle nicht platzt (>50.000 Zeichen)!
        slim_sessions = []
    ...
    with c2:
        if st.button("Session starten", type="primary", use_container_width=True, disabled=not can_save):
            if can_save:
                ...
                # Wir übergeben hier GANZ GANZ WICHTIG die Funktionstabelle, um doppelte Paare zu berechnen.
                trainings_sessions_all = [s for s in st.session_state.sessions_list if not s.get("is_liga") and not s.get("is_wettkampf")]
                if spielmodus in ["Koop 2vs2 (Up & Down)", "Standard-Training (Einzel + Coop)"]: 
                    get_or_create_teams(new_session, trainings_sessions_all)
...
with tab_regeln:
    ...
    with st.container(border=True):
        ...
        * **Fehler Korrigieren:** Über den "✏️ Korrigieren" Button direkt in der laufenden Session kann die zuletzt gespielte Runde sofort repariert werden (fat-finger errors).""")# INSTRUKTION: DIESE REGELN DÜRFEN BEI CODE-UPDATES NIEMALS VERLETZT WERDEN
# 1. BACKUPS: Das Rolling-Backup in Google Sheets darf maximal 20 Einträge umfassen (ältere löschen).
# 2. JSON-EXPORT: Vor jedem json.dumps() MUSS die Hilfsfunktion make_serializable() aufgerufen werden, um Tupel/Datumsformate abzusichern!
# 3. KOOP-TEAMS: Es dürfen niemals exakt gleiche 2er-Teams aus der vorherigen Session gebildet werden.
# 4. ANTI-DOPPEL-PAUSE: Das Freilos in Runde 1 muss rotieren. Wer im letzten Match pausiert hat, darf nicht nochmal aussetzen.
